<template>
  <div>
    <h2 id="page-heading" data-cy="PhotoHeading">
      <span v-text="$t('articleApp.photo.home.title')" id="photo-heading">Photos</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('articleApp.photo.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'PhotoCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('articleApp.photo.home.createLabel')"> Create a new Photo </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && photos && photos.length === 0">
      <span v-text="$t('articleApp.photo.home.notFound')">No photos found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="photos">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('url')">
              <span v-text="$t('articleApp.photo.url')">Url</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'url'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('banner.id')">
              <span v-text="$t('articleApp.photo.banner')">Banner</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'banner.id'"></jhi-sort-indicator>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="photo in photos" :key="photo.id" data-cy="entityTable">
            ...
            <td>{{ photo.url }}</td>
            <td>
              <div v-if="photo.banner">
                <router-link :to="{ name: 'BannerView', params: { bannerId: photo.banner.id } }">{{ photo.banner.id }}</router-link>
              </div>
            </td>
            <td class="text-right">
              <div class="btn-group">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="articleApp.photo.delete.question" data-cy="photoDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-photo-heading" v-text="$t('articleApp.photo.delete.question', { id: removeId })">
    ...
    <div v-show="photos && photos.length > 0">
